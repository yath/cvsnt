...
static void RCS_output_diff_options (const char *opts, const char *rev1, const char *rev2, const char *workfile);
...
static void call_diff_flush_output();
...
static void call_diff_arg (const char *s)
{
    call_diff_add_arg (s);
}

static void call_diff_add_arg (const char *s)
{
...
static void call_diff_write_output (const char *text, size_t len)
{
    if (len > 0)
	cvs_output (text, len);
}
...
static void call_diff_write_stdout (const char *text)
{
    cvs_output (text, 0);
}
...
static void call_diff_error (const char *format, const char *a1, const char *a2)
{
...
static FILE *call_diff_fopen(const char *file, const char *mode)
{
	return CVS_FOPEN(file,mode);
}

static int call_diff_open(const char *file, int mode, ...)
{
	return CVS_OPEN(file,mode);
}

static int call_diff_stat(const char *fn, struct stat *st)
{
	return CVS_STAT(fn,st);
}
...
int RCS_merge(RCSNode *rcs, const char *path, const char *workfile, const char *options, const char *rev1, const char *rev2, int conflict_3way, mode_t *mode)
{
	...
	/* They are the same symbolic revision - do nothing */
	if(!strcmp(xrev1,xrev2))
	{
		xfree(xrev1);
		xfree(xrev2);
		return 3; /* No difference */
	}
	...
	{
		static char newopts[128];
		is_unicode = 1;
		kf.flags&=~KFLAG_ENCODED;
		options = RCS_rebuild_options(&kf, newopts);
	}
	else
		is_unicode = 0;
	if(!options || !*options)
    ...
    if (RCS_checkout (rcs, NULL, xrev2, rev2, options, tmp2,
		      (RCSCHECKOUTPROC)0, NULL, &lmode2))
    {
	cvs_outerr ("rcsmerge: co failed\n", 0);
	error_exit();
    }

	/* getting mode from 2nd revision (target revision) */
	*mode = lmode2;
...
/*	else if (retval == 3)   
    ...
    if (diffout)
	{
		...
		if(kf.flags&KFLAG_MAC)
			...
			crlf=ltCrLf;
		else if(kf.flags&(KFLAG_BINARY|KFLAG_UNIX))
			crlf=ltLf;
		if(is_unicode || (crlf!=CRLF_DEFAULT && !server_active))
		{
			CCodepage cdp;
			char *unibuf=NULL;
			size_t unisiz,unilen;
			FILE *f;
			TRACE(2,"Transcoding %s to %s",PATCH_NULL(wf),PATCH_NULL(workfile));
			...
			cdp.BeginEncoding(CCodepage::NullEncoding,kf.encoding);
			if(cdp.OutputAsEncoded(fileno(f),unibuf,unilen,crlf))
			...
			copy_file (diffout, workfile, 1, 1);
	}
...
RCS file: ", 0);
    ...
    cvs_output (rev1, 0);
    cvs_output ("\n", 1);
    status = RCS_checkout (rcsfile, NULL, (char*)rev1, NULL, (char*)options, tmpfile1,
	...
	cvs_output (rev2, 0);
	cvs_output ("\n", 1);
	status = RCS_checkout (rcsfile, NULL, (char*)rev2, NULL, (char*)options,
...
int diff_exec (const char *file1, const char *file2, const char *label1, const char *label2, const char *options, const char *out)
{
    ...
    args = (char*)xmalloc (strlen (options) + 10);
